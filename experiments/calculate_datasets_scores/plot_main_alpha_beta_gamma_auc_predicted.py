...
def main(dp: int = 100, cv_folds: int = 5, out_png: str = "six_panel_model_dataset_eval.png"):
    ...
    A_md = pd.read_csv(f"model_dataset_alpha_values_data_percentage_{dp}.csv", index_col=0)
    B_md = pd.read_csv(f"model_dataset_beta_values_data_percentage_{dp}.csv",  index_col=0)
    ...
    beta_D  = read_series_csv(f"dataset_beta_values_data_percentage_{dp}.csv",  "beta").reindex(dataset_names)
    gamma_D = read_series_csv(f"dataset_gamma_values_data_percentage_{dp}.csv", "gamma").reindex(dataset_names)
    # per-(dataset, model) etas: rows=datasets, cols=models
    ETA_Dm = pd.read_csv(f"dataset_eta_values_data_percentage_{dp}.csv", index_col=0)
    ...
    beta_m  = read_series_csv(f"model_beta_values_data_percentage_{dp}.csv",  "beta").reindex(model_names)
    gamma_m = read_series_csv(f"model_gamma_values_data_percentage_{dp}.csv", "gamma").reindex(model_names)
    # per-(model, dataset) thetas: rows=models, cols=datasets
    THETA_mD = pd.read_csv(f"model_theta_values_data_percentage_{dp}.csv", index_col=0)
    ...
    print(f"{np.mean((np.array(md_pred)-np.array(md_actual))**2):.6f} mean squared error for Model–Dataset–k–j points")
    scatter_ax(ax_md, md_pred, md_actual, "Model–Dataset–k–j points (pairwise coeffs)")

    print(f"{np.mean((np.array(m_pred)-np.array(m_actual))**2):.6f} mean squared error for Model–k–j–Dataset points")
    scatter_ax(ax_m,  m_pred,  m_actual,  "Model–k–j–Dataset points (α_D + β_m, γ_m)")

    print(f"{np.mean((np.array(d_pred)-np.array(d_actual))**2):.6f} mean squared error for Dataset–k–j–Model points")
    ...
    fig.suptitle(f"Actual vs Predicted AUC (dp={dp}) with per-(m,D,k,j) points and Coefficient Distributions", y=0.98)
