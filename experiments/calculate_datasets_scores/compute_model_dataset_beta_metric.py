...
def main():
    ...
    for is_models_homophility in [True, False]:
        for is_datasets_homophility in [True, False]:
            suffix = ""
            if is_models_homophility:
                models_names = ModelFactory.MODELS_HOMOPHILITY
                suffix += "_models_homophility"
            else:
                models_names = ModelFactory.MODELS_NON_HOMOPHILITY
                suffix += "_models_non_homophility"

            if is_datasets_homophility:
                dataset_names = ModelFactory.DATASETS_HOMOPHILITY
                suffix += "_datasets_homophility"
            else:
                dataset_names = ModelFactory.DATASETS_NON_HOMOPHILITY
                suffix += "_datasets_non_homophility"
            ...
            for dp in data_percentages:
                ...
                beta_out_csv = f"model_dataset_beta_values{suffix}_data_percentage_{dp}.csv"
                beta_mat.to_csv(beta_out_csv, index=True)
                print(f"[OK] Wrote {beta_out_csv}")

                gamma_out_csv = f"model_dataset_gamma_values{suffix}_data_percentage_{dp}.csv"
                gamma_mat.to_csv(gamma_out_csv, index=True)
                print(f"[OK] Wrote {gamma_out_csv}")

                alpha_out_csv = f"model_dataset_alpha_values{suffix}_data_percentage_{dp}.csv"
