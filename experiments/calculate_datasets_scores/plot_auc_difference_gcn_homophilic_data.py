...
if __name__ == '__main__':
    ...
    models_names = ModelFactory.MODELS_HOMOPHILITY + [
        ModelFactory.Complete_Random_Forest_NAME,
        ModelFactory.Complete_Gradient_Boosting_Classifier_Name,
        ModelFactory.Mean_Gradient_Boosting_Classifier_Name,
    ...
    homophily_scores_df = pd.read_csv("homophility_scores.csv")
    homophily_scores_df["datasets"] = homophily_scores_df["datasets"].str.strip()

    # build a score lookup
    score = homophily_scores_df.set_index("datasets")["scores"]

    datasets_names = ModelFactory.DATASETS_HOMOPHILITY

    datasets_names = sorted(datasets_names, key=lambda x: score[x], reverse=True)

    # Create a dictionary mapping dataset names to their original dictionaries
    dataset_dict = {dataset['name']: dataset for dataset in datasets}

    # Reorder datasets based on datasets_names
    datasets = [dataset_dict[name] for name in datasets_names if name in dataset_dict]
    ...
    fig.legend(
        ...
        bbox_to_anchor=(0.5, 1.13),  # Shift legend upward (increase value to move higher)
    ...
    plt.savefig(f"{metric}_difference_gcn_homophilic_data.png", bbox_inches='tight', dpi=300)
