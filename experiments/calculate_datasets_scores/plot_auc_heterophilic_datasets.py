...
def make_plot(dataset_name: str, dataset_plot_name: str, metric: str, metric_title: str,
    ...
    for model_name in models_names:
        for features_amount in features_amounts_filtered_max:  # skip the first one, as it is total features
            # scores for model with features over all the cvs
            scores_max = []
            for cv in range(cvs):
    ...
    for idx, model_name in enumerate(models_names):
        ...
        # Plot the difference
            ...
            ax.plot(features_amounts_filtered_max, diff_means, label=ModelFactory.display_name(model_name),
                    color=ModelFactory.color_for(model_name))
            # plot errors
...
if __name__ == '__main__':
    ...
    num_rows = 2  # Calculate the required number of rows
    ...
    for j, dataset in enumerate(datasets):
        ...
        row, col = divmod(dataset_index, num_cols)  # Compute row and col position dynamically
    ...
    plt.savefig(f"{metric}_heterophilic_datasets.png", bbox_inches='tight', dpi=300)
