...
if __name__ == '__main__':
    ...
    G_md = pd.read_csv(f"model_dataset_gamma_values_data_percentage_100.csv", index_col=0)
    ...
    for j, dataset in enumerate(datasets):
        ...
        gamma_values[ModelFactory.DAE_NAME][dataset_name] = G_md.loc[ModelFactory.DAE_NAME, dataset_name]
        gamma_values[ModelFactory.GAT_NAME][dataset_name] = G_md.loc[ModelFactory.GAT_NAME, dataset_name]
        gamma_values[ModelFactory.GCN_NAME][dataset_name] = G_md.loc[ModelFactory.GCN_NAME, dataset_name]
        gamma_values['Best Model'][dataset_name] = G_md.loc[best_model_name, dataset_name]

    # Models to plot (order controls legend order)
    models = [
        ModelFactory.DAE_NAME,
        ModelFactory.GAT_NAME,
        ModelFactory.GCN_NAME,
    ...
    for m in models:
        ...
        for ds in datasets:
            ds_name = ds['name']
            # get homophily x
            if ds_name not in datasets_to_homophilities:
                continue
            x = datasets_to_homophilities[ds_name]

            # get gamma y for this model & dataset
            if ds_name not in gamma_values[m]:
                continue
            y = gamma_values[m][ds_name]

            xs.append(x + offsets[m])  # apply small jitter
            ys.append(y)
            labels.append(ds.get('plot_name', ds_name))
    ...
    plt.ylabel('Gamma (γ)', fontsize=14)
    plt.title('Gamma vs. Homophily per Dataset and Model', fontsize=16)
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(title='Model', frameon=True)

    # Optional: annotate each point with dataset short name (comment out if too busy)
    # for m in models:
    #     for ds in datasets:
    #         ds_name = ds['name']
    #         if ds_name in datasets_to_homophilities and ds_name in gamma_values[m]:
    #             x = datasets_to_homophilities[ds_name] + offsets[m]
    #             y = gamma_values[m][ds_name]
    #             plt.text(x, y, ds.get('plot_name', ds_name), fontsize=8, ha='left', va='bottom')

    plt.tight_layout()
    plt.savefig('gamma_vs_homophily_scatter.png', dpi=300, bbox_inches='tight')
    plt.show()
