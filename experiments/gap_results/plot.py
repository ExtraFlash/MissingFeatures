...
def make_plot(dataset_name: str, dataset_plot_name: str, metric: str, metric_title: str,
    ...
    for model_name in models_names:
        for features_amount in feautres_amounts_filtered:
            ...
            # get auc results (scores_max are the max results)
            scores = results[model_name][features_amount]

            results_max[model_name][features_amount] = [scores_max[i] - scores[i] for i in range(cvs)]
...
if __name__ == '__main__':
    ...
    datasets: list = config['datasets'][:-2]  # Not include the last 2 datasets for now
