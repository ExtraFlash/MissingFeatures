...
def create_data(n_samples, n_features, n_rank, positive_label_ratio, n_categorical, ratios, n_categories=2, n_output=1):
    """
    ...
    for i in range(n_features):
        ...
        # Determine threshold for labels to achieve the given ratio of positive samples
    sorted_probs, _ = torch.sort(probs.view(-1))
    threshold_index = int(n_samples * (1 - positive_label_ratio))
    label_threshold = sorted_probs[threshold_index].item()

    data['label'] = (probs > label_threshold).int().view(-1)

    # print the ratio of positive samples
    print(f'ratio: {(probs > label_threshold).sum().item() / n_samples}')
...
positive_label_ratio = config['positive_label_ratio']
...
create_data(n_samples, n_features, n_rank, positive_label_ratio, n_categorical, ratios)
