...
def create_data(n_samples, n_features, n_rank, n_categorical, n_categories=2, n_output=1):
    """
    ...
    for i in range(n_categorical):
        input_data[:, i] = (input_data[:, i] > 0).int()
    # hidden matrix
    hidden_matrix = torch.randn(n_features, n_output)
    # output layer
    output = torch.mm(input_data, hidden_matrix)
    # apply sigmoid
...
create_data(n_samples, n_features, n_rank, n_categorical)
